...
$lowestSeatID = $allSeatIDs | sort -Descending | Select-Object -last 1

for ($sequence=$lowestSeatID; $sequence -lt $highestSeatID; $sequence++)
{
    if ($allSeatIDs -match $sequence)
    {
        #boarding pass has been taken
    }
    else
    {
        #empty seat found
        Write-Host -ForegroundColor Green "Seat ID $sequence is unclaimed."
    }   
}